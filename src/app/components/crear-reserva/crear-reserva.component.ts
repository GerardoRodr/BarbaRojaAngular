...
export class CrearReservaComponent implements OnInit {
  ...
  constructor(
    private apiServicio: ServiciosService,
  ...
  crearReserva() {
    ...
    if (fechaCompleta < new Date()) {
      Swal.fire({
        title:
          '<strong class="text-body">La fecha no puede ser anterior a hoy</strong>',
        icon: 'warning',
        html: `
  ...
  showDialog(
    tipoIcono: 'error' | 'warning' | 'success',
    mensaje?: string,
    titulo?: string
  ): void {
    Swal.fire({
      title: `<strong class="text-body">${
